...
export default async function SubmissionDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ screening?: string }> }) {
  ...
  const failedChecks = submission.ai_checks.filter((check) => check.verdict === "fail").length;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(20rem,.75fr)]">
          ...
          <aside className="grid content-start gap-4">
            <Card><CardHeader><h2 className="text-sm font-semibold">Review snapshot</h2></CardHeader><CardContent className="grid gap-4 text-sm"><div className="flex justify-between"><span className="text-slate-500">Rules checked</span><strong>{submission.ai_checks.length}</strong></div><div className="flex justify-between"><span className="text-slate-500">Potential issues</span><strong>{failedChecks}</strong></div><div className="flex justify-between"><span className="text-slate-500">Review decisions</span><strong>{submission.reviews.length}</strong></div><div className="flex justify-between"><span className="text-slate-500">Comments</span><strong>{submission.comments.length}</strong></div></CardContent></Card>
