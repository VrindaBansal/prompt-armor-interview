...
export default async function UpdatePasswordPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <div>
      ...
      <p className="mt-3 text-sm leading-6 text-slate-600">Use at least 12 characters and avoid passwords used for other services.</p>
      ...
      <form action="/auth/update-password" className="mt-8 grid gap-5" method="post">
        <Field autoComplete="new-password" label="New password" maxLength={1024} minLength={12} name="password" required type="password" />
        <Field autoComplete="new-password" label="Confirm new password" maxLength={1024} minLength={12} name="password_confirmation" required type="password" />
